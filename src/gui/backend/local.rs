...
/// make it `!Send + !Sync`.  We always access it through a `Mutex`, so sending
/// the *wrapper* across threads is safe.
struct AutoGuiWrapper(RustAutoGui);

// SAFETY: Access is serialised by the `Mutex` in `LocalBackend`.
...
impl DisplayCapability for LocalBackend {
    async fn screenshot(&self) -> Result<Screenshot, GuiError> {
        ...
        {
            let mut autogui = self.autogui.lock().unwrap();
            autogui
...
impl InputCapability for LocalBackend {
    ...
    async fn key_combo(&self, keys: &[String]) -> Result<(), GuiError> {
        let autogui = self.autogui.lock().unwrap();
        match keys.len() {
            2 => autogui.keyboard_multi_key(&keys[0], &keys[1], None),
            3 => autogui.keyboard_multi_key(&keys[0], &keys[1], Some(&keys[2])),
            _ => {
