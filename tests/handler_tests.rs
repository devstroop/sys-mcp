...
async fn test_gui_screenshot() {
    ...
    let result = handle_tool_call(&client, "gui_screenshot", json!({})).await;
...
async fn test_gui_screenshot_region_missing_args() {
    let client = test_client();
    let result = handle_tool_call(&client, "gui_screenshot_region", json!({"x": 0})).await;
...
async fn test_gui_get_screen_size() {
    let client = test_client();
    let result = handle_tool_call(&client, "gui_get_screen_size", json!({})).await;
    assert!(!result.is_error.unwrap_or(false));
...
async fn test_gui_list_monitors() {
    let client = test_client();
    let result = handle_tool_call(&client, "gui_list_monitors", json!({})).await;
    assert!(!result.is_error.unwrap_or(false));
    let text = result.content[0].text.as_deref().unwrap_or("");
...
async fn test_gui_click() {
    let client = test_client();
    let result = handle_tool_call(&client, "gui_click", json!({"x": 100, "y": 200})).await;
    assert!(!result.is_error.unwrap_or(false));
    let text = result.content[0].text.as_deref().unwrap_or("");
...
async fn test_gui_click_missing_y() {
    let client = test_client();
    let result = handle_tool_call(&client, "gui_click", json!({"x": 100})).await;
    assert_eq!(result.is_error, Some(true));
}
...
async fn test_gui_double_click() {
    let client = test_client();
    let result = handle_tool_call(&client, "gui_double_click", json!({"x": 50, "y": 60})).await;
    assert!(!result.is_error.unwrap_or(false));
}
...
async fn test_gui_mouse_move() {
    let client = test_client();
    let result = handle_tool_call(&client, "gui_mouse_move", json!({"x": 10, "y": 20})).await;
    assert!(!result.is_error.unwrap_or(false));
    let text = result.content[0].text.as_deref().unwrap_or("");
...
async fn test_gui_mouse_position() {
    let client = test_client();
    let result = handle_tool_call(&client, "gui_mouse_position", json!({})).await;
    assert!(!result.is_error.unwrap_or(false));
...
async fn test_gui_type_text() {
    let client = test_client();
    let result = handle_tool_call(&client, "gui_type_text", json!({"text": "hello world"})).await;
    assert!(!result.is_error.unwrap_or(false));
    let text = result.content[0].text.as_deref().unwrap_or("");
...
async fn test_gui_press_key_simple() {
    let client = test_client();
    let result = handle_tool_call(&client, "gui_press_key", json!({"key": "return"})).await;
    assert!(!result.is_error.unwrap_or(false));
...
async fn test_gui_press_key_combo() {
    let client = test_client();
    let result = handle_tool_call(&client, "gui_press_key", json!({"key": "ctrl+c"})).await;
    assert!(!result.is_error.unwrap_or(false));
}
...
async fn test_gui_press_key_missing_key() {
    let client = test_client();
    let result = handle_tool_call(&client, "gui_press_key", json!({})).await;
    assert_eq!(result.is_error, Some(true));
}
...
async fn test_gui_get_clipboard() {
    let client = test_client();
    let result = handle_tool_call(&client, "gui_get_clipboard", json!({})).await;
    assert!(!result.is_error.unwrap_or(false));
}
...
async fn test_gui_set_clipboard() {
    let client = test_client();
    let result = handle_tool_call(&client, "gui_set_clipboard", json!({"text": "test"})).await;
    assert!(!result.is_error.unwrap_or(false));
    assert_eq!(
        result.content[0].text.as_deref().unwrap_or(""),
        "Clipboard set."
...
async fn test_gui_list_windows() {
    let client = test_client();
    let result = handle_tool_call(&client, "gui_list_windows", json!({})).await;
    assert!(!result.is_error.unwrap_or(false));
}
...
async fn test_gui_get_active_window_errors() {
    let client = test_client();
    let result = handle_tool_call(&client, "gui_get_active_window", json!({})).await;
    assert_eq!(result.is_error, Some(true));
}
...
async fn test_gui_focus_window_by_id() {
    let client = test_client();
    let result = handle_tool_call(&client, "gui_focus_window", json!({"window_id": 1})).await;
    assert!(!result.is_error.unwrap_or(false));
}
...
async fn test_gui_focus_window_no_args() {
    let client = test_client();
    let result = handle_tool_call(&client, "gui_focus_window", json!({})).await;
    assert_eq!(result.is_error, Some(true));
}
...
async fn test_gui_find_windows() {
    let client = test_client();
    let result = handle_tool_call(&client, "gui_find_windows", json!({"query": "Firefox"})).await;
    assert!(!result.is_error.unwrap_or(false));
    let text = result.content[0].text.as_deref().unwrap_or("");
...
async fn test_gui_system_info() {
    let client = test_client();
    let result = handle_tool_call(&client, "gui_system_info", json!({})).await;
    assert!(!result.is_error.unwrap_or(false));
    let text = result.content[0].text.as_deref().unwrap_or("");
...
async fn test_gui_wait() {
    let client = test_client();
    let result = handle_tool_call(&client, "gui_wait", json!({"ms": 1})).await;
    assert!(!result.is_error.unwrap_or(false));
    let text = result.content[0].text.as_deref().unwrap_or("");
...
async fn test_gui_screenshot_region_missing_width() {
    let client = test_client();
    let result = handle_tool_call(&client, "gui_screenshot_region", json!({"x": 0, "y": 0})).await;
    assert_eq!(result.is_error, Some(true));
}
...
async fn test_gui_double_click_missing_args() {
    let client = test_client();
    let result = handle_tool_call(&client, "gui_double_click", json!({"x": 50})).await;
    assert_eq!(result.is_error, Some(true));
}
...
async fn test_gui_scroll_missing_direction() {
    let client = test_client();
    let result = handle_tool_call(&client, "gui_scroll", json!({"x": 0, "y": 0})).await;
    assert_eq!(result.is_error, Some(true));
}
...
async fn test_gui_type_text_empty() {
    let client = test_client();
    let result = handle_tool_call(&client, "gui_type_text", json!({"text": ""})).await;
    assert!(!result.is_error.unwrap_or(false));
}
...
async fn test_gui_press_key_combo_multi_modifier() {
    let client = test_client();
    let result =
        handle_tool_call(&client, "gui_press_key", json!({"key": "ctrl+alt+delete"})).await;
    assert!(!result.is_error.unwrap_or(false));
}
...
async fn test_gui_move_resize_window_no_args() {
    let client = test_client();
    let result = handle_tool_call(&client, "gui_move_resize_window", json!({})).await;
    assert_eq!(result.is_error, Some(true));
}
